...
{
    internal class OwnerMarketScreen : IScreen
    {
        MarketSaleManager _marketSaleManager;
        public OwnerMarketScreen()
        {
            _marketSaleManager = new();
        }
        public void Show()
        {
            while (true)
        ...
        void ShowOptions()
        {
            Console.WriteLine("------------OWNER MARKET PANEL------------");
        ...
        void TodaysRevenue()
        {
            var result = _marketSaleManager.TotalRevenueByDate(DateTime.Now.Date,DateTime.Now.AddDays(1).Date);
            if(result.Any())
            {
        ...
        void RevenueByDates()
        {
            ...
            var result = _marketSaleManager.TotalRevenueByDate(start,end);
        ...
        void TotalRevenue()
        {
            var result = _marketSaleManager.TotalRevenue();
            Console.WriteLine($"Total Revenue so far is {result.ToString("00.##")}");
        }

        void ListAllSales()
        {
            var result = _marketSaleManager.GetAllSales();
